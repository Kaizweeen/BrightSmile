...
- **Logo: provided by Kai.** A deep teal and navy tooth with a gold swoosh and a white sparkle, above a smile arc, on a cream circle; wordmark "BRIGHT SMILE" in heavy navy caps over "DENTAL CLINIC" in letterspaced teal caps. The file is committed at `public/brand/logo.png` (1024px square master, with `logo.webp` alongside it for page use) and drives the app icons at `src/app/icon.png` (256px) and `src/app/apple-icon.png` (180px). Each raster carries its origin in its metadata. Because the square carries a blurred clinic photo behind the circle, it is used where it has room (app icon, and the landing page later), while small chrome keeps the type lockup. A version of the mark alone on a transparent background would let it into the smaller places too.
- **Interface style: pinned by the owner to his Planorama project.** Structure, spacing, radii, shadow, component classes and the Inter with Poppins pairing come from that project's `app/globals.css`, copied rather than approximated. The colour comes from the logo: teal `#12727E` as the working colour, navy `#0F3A52` as ink, gold `#C9A24B` as accent, cream `#F7F3EA` as the canvas, white cards. Light only, as Planorama is. This pinned style outranks any generated design direction.
