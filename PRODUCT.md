...
- **Logo: provided by Kai.** A deep teal and navy tooth with a gold swoosh and a white sparkle, above a smile arc, on a cream circle; wordmark "BRIGHT SMILE" in heavy navy caps over "DENTAL CLINIC" in letterspaced teal caps. The file is committed at `public/brand/logo.png` (1254px square) and drives the app icons at `src/app/icon.png` and `src/app/apple-icon.png`. Because the square carries a blurred clinic photo behind the circle, it is used where it has room (app icon, and the landing page later), while small chrome keeps the type lockup. A version of the mark alone on a transparent background would let it into the smaller places too.
